...
## Wave 1 — Foundation (depends: none)
...
- **T1.2 WS protocol spec v1** — `docs/05-protocol.md` exhaustive: envelope `{v,type,id,seq,payload,hmac}`, all message types incl. `fight_input`/`fight_input_ack`/`fight_snapshot`/`fight_submit`, per-frame `id` semantics, HMAC, resume order, idempotency, error codes.
  - Deps: T1.1 · Accept: `test_protocol.py` validates example frames (HMAC, seq, resume, 64KB cap) — satisfies FR-7.
...
- **T1.6 Living-docs scaffold + drift gate** — materialize `docs/`; `uv run docs:check` (regenerate derivable docs + diff, exit non-zero on drift); code→doc manifest with inverse check (unlisted file = fail; empty doc mapping = fail).
...
## Wave 2 — Deterministic core (depends: Wave 1)
...
- **T2.2 Combat sim spec + engine rules** — `rules.py` + `catalog/minimal/` seed; damage `max(1, atk−def)` × mechanical multipliers; pinned stamina/posture/i-frame/parry values; fight-length bound (10min/36k ticks); sector/shrine/death/market/boss-skill rules.
  - Deps: T2.1 · Accept: unit tests (frame-exact windows, behavior-table determinism, descent invariants) — satisfies FR-1, FR-3, FR-4.
- **T2.3 Shared sim core (dual impl + conformance)** — `server/app/game/sim/core.py` + `client/scripts/sim_core.gd`; `sim_conformance.py` ≥2000 seeded cases, byte-identical; spec pins input-buffering-in-sim, RNG isolation, canonical `state_hash`.
  - Deps: T2.2 · Accept: corpus green both languages — satisfies NFR-1, FR-2.
- **T2.4 Seeded floor generator** — 4-room template; shrine floor-1 guaranteed + excluded from floors 2–4 pool; ≥1 loot/event per sector; difficulty band ±25%; `place_enemy` (Wave-2 commit path); token caps (floor ≤800/encounter ≤400/narrative ≤600).
  - Deps: T2.2 · Accept: 1000-seed invariants (boss+shrine+loot/event); reachability; `place_enemy` sole append path.
- **T2.5 Persistence + session service + retention** — SQLite WAL/aiosqlite/single-writer; tables (`sessions/pregen_cache/generated_lore/action_ids`); retention cascade; `headless_player.py` (scenarios).
  - Deps: T2.1, T2.4 · Accept: save/load round-trip; retention removes all rows across stores.

**Wave 2 done-claim**: `headless_player.py` walks a floor, fights via sim (re-sim → result), lights shrine, shops, returns home, levels a skill; `sim_conformance.py` green.

## Wave 3 — Director (depends: Wave 2)

- **T3.1 Agent graph + checkpointer + concurrency** — StateGraph (`route_intent` → subgraphs), `interrupt()`, checkpointer (SQLite dev/Postgres prod); engine-first routing; per-session graph mutex; `RemoveMessage` trim; narrate grounded in typed facts.
  - Deps: T2.5 · Accept: interrupt/resume no re-bill; `talk` during narration → busy — satisfies FR-5.1, FR-5.4.
- **T3.2 Tool layer (engine gateway)** — `commit_encounter` (single write path, synchronous critical section, verdict-required with synthetic fallback, placement-validated); `get_player_build/get_floor_state/get_fight_facts/save_lore_fact` (provenance forcing, call cap); no `roll_dice`.
  - Deps: T2.4 · Accept: mutation + race tests prove sole-mutator — satisfies NFR-3, NFR-4, Invariant 2/3.
...
- **T3.4 Content generation pipeline** — pre-gen off critical path; cache key + LRU + TTL; cold-cache fallback; token-budget enforcement; generation tracker → terminal frame.
  - Deps: T3.1 · Accept: cache-hit descent; hung gen → `generation_failed` + fallback — satisfies FR-5.4.
- **T3.5 Session resume + retention + queue ownership** — resume → `state_sync` first → queued actions → `narrative_replay`; client-owned queue; checkpoint prune (last 50/thread).
  - Deps: T3.1, T2.5 · Accept: crash-resume exact state; no double-apply — satisfies FR-2.4, NFR-6.
- **T3.6 Verification-agent loop** — 4 judges gate committed content only (streamed narrative structurally grounded); bounded repair ≤2 → fallback; verdicts traced.
...
- **T4.1 Catalog seed data** — full MVP catalog (≥60 parts, ≥30 affixes, ≥40 items, ≥10 themes, ≥30 lore; 3 classes/10 enemies/3 bosses/boss skills/market stock).
  - Deps: T2.2 · Accept: referential integrity, counts ≥ targets — satisfies FR-6.1.
- **T4.2 Qdrant + hybrid retrieval** — docker-compose; indexer (one doc/record + provenance); retriever (dense+BM25+RRF+payload filters); golden set (20).
  - Deps: T4.1 · Accept: recall@5 ≥70% — satisfies FR-6.2.
- **T4.3 Variant composition + clamp layer** — `compose_variant` (retrieval ≤1.5k tokens → LLM → clamp); commit via `commit_encounter`.
  - Deps: T4.2, T3.2 · Accept: ≥95% schema-valid, 100% clamp-enforced, ≤2 calls — satisfies FR-6.3, FR-5.2.
- **T4.4 Game-design corpus ingestion** — FREE-LEGAL set with license+provenance; license gate checks redistribution AND MIT-compat (Ironsworn/RogueBasin → link-only); corpus-as-untrusted-data.
  - Deps: T4.2 · Accept: every record has license/source/domain; hostile-corpus injection blocked — satisfies FR-6.3, NFR-5.
- **T4.5 Safety: lore quarantine + moderation** — ring buffer 500/session; provenance honored on re-embed; generated lore wrapped as data.
...
## Wave 5 — Client (depends: Wave 2 protocol + mock; Wave 3/4 content optional)

- **T5.1 Isometric rendering** — TileMapLayer `TILE_SHAPE_ISOMETRIC` + separate layers + Y-sort (feet origin); screen↔iso; placeholder shapes.
  - Deps: T1.4 · Accept: same seed → identical rendered floor (xvfb screenshot diff).
- **T5.2 GDScript sim core mirror + conformance re-run** — wire `sim_core.gd`; re-run `sim_conformance.py` in-engine; no `randf`/wall-clock in sim.
  - Deps: T2.3, T5.1 · Accept: corpus green with in-engine GDScript.
- **T5.3 Real-time combat** — hitbox/hurtbox feedback-only; hit resolution sim-computed; input buffering (200ms); fight-log streaming (batched + acked, idempotent server append); `fight_submit` (no full log); `verified:false` cap ≤2; reconnect/crash resume via persisted prefix.
...
- **T5.5 Reconnection + error handling** — reconnect overlay; `state_sync` wholesale replace; error toasts; fight-mid-flight resume (persisted prefix).
...
## Wave 6 — Hardening (depends: Wave 5)

- **T6.1 LangSmith eval suite** — record corpus via scripted sessions; datasets (narrative/rule/balance/pacing/verifier); code evaluators; thresholds (NFR-8).
  - Deps: Wave 3–5 · Accept: eval run produces report; seeded regression fails balance gate.
- **T6.2 Latency + cost verification** — `playtest.py` measures p50/p95/TTFT/WS/re-sim; re-sim in `asyncio.to_thread`; tune routing/caching.
  - Deps: T6.1 · Accept: budgets met or documented — satisfies NFR-2.
- **T6.3 Anti-tamper hardening verification** — fuzz malformed/replayed/forged frames; token-theft test; client audit (no content of value); telemetry hook; WSS/TLS documented.
  - Deps: Wave 5 · Accept: fuzz green; audit checklist evidenced — satisfies NFR-5.
- **T6.4 Docs, quickstart, final balance pass** — complete GDD; final balance vs T6.1 gates; README quickstart <15min; runbook.
